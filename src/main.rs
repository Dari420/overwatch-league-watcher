...
fn main() {
    ...
    loop {
        if size != String::from("100") || size != String::from("125") {
            println!("Please enter your windows scale (100 or 125)");
            stdin()
                .read_line(&mut size)
                .expect("error reading line, aborting");
        }
        else {
            break;
        }
...
fn open_gigabrain_overwatch(size: String) {
    let mut enigo = Enigo::new();
    println!("Opening overwatch league.");
    if size == String::from("100") {
